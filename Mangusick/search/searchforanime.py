...
# НЕ ЕБУ КАК ЭТО РАБОТАЕТ, НО СПИЗДИЛ ИЗ ССЫЛКИ КОТОРУЮ ОСТАВЛЯЛ В ФАЙЛЕ anime.py (по идее это поисковик по API shikimori) Api тот же использовать будем потому что выбора аниме больше, а с мангой не ебу что делать

from aiogram import InlineKeyboardButton, InlineKeyboardMarkup
from users import users, clear_state
from mangaanime import anime
import re
